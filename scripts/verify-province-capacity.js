...
(async () => {
  ...
  const industryNote = await page.locator("#power-industry-note").innerText();
  assert(industryNote.includes("202606") && industryNote.includes("当月"), "industry monthly chart is not on the latest period");
  assert((await page.locator("#power-industry-year-chart").count()) === 1, "industry monthly chart missing");
  ...
  assert(await page.locator("#province-capacity-detail-body tr").count() >= 18, "province monthly capacity table too short");
  const tableScroll = await page.locator(".province-capacity-table").first().evaluate((node) => ({
