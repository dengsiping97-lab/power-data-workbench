...
for (const company of listed) {
  ...
  assert(Math.abs(published.power - num(source["上市公司水电估算亿kWh"])) < 1e-9, `${company}: power differs from source`);
...
assert(changjiang?.power > 50 && changjiang?.yoy !== null, "Changjiang W29 full-scope power/yoy missing");
